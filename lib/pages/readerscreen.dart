...
class _MyReaderPageState extends State<ReaderScreen> {
  ...
  void initReaderScreen(Store<AppState> store) {
    ...
    _statusTimer = Timer.periodic(Duration(seconds: AppConstants.STATUS_TIME_UPDATE_INTERVAL_SECONDS), (Timer t) => _updateStatusTime());
  ...
  _navigateToScrollPositionForFirstTime() {
    ...
    final bool savePos = StoreProvider.of<AppState>(context).state.options.saveScrollPosition;
    final double loadedScrollOffset = StoreProvider.of<AppState>(context).state.options.scrollOffset[id.toString()].scrollOffset;
    final double offset = savePos ? loadedScrollOffset : 0.0;
    _controller.animateTo(offset, duration: new Duration(milliseconds: 500), curve: Curves.ease);
    StoreProvider.of<AppState>(context).dispatch(UpdateStatusScrollPercentageAction(ScrollInfo(id, offset, _controller.position.maxScrollExtent)));
  }
  ...
  _updateScrollPosition() {
    ...
    StoreProvider.of<AppState>(context).dispatch(UpdateStatusScrollPercentageAction(ScrollInfo(id, offset, maxOffset)));
  }
  ...
  String _calculateScrollPerc(double offset, double maxoffset) {
    final double scrollPerc = (offset != 0.0) ? (offset / maxoffset) * 100 : 0.0;
  ...
  void _updateStatusTime() {
    ...
    StoreProvider.of<AppState>(context).dispatch(UpdateStatusTimeAction(timeString)); 
  }
  ...
  Widget build(BuildContext context) {
    ...
    var result = WillPopScope (
      onWillPop: () {
        StoreProvider.of<AppState>(context).dispatch(ClearReaderOptionsToggleAction());
      ...
      child: StoreConnector<AppState, _ViewModel>(
          ...
          body: Container(
            decoration: BoxDecoration(
              image: new DecorationImage(   
                colorFilter: new ColorFilter.mode(Colors.black.withOpacity(AppConstants.READERTHEME_BACK_OPACITY), BlendMode.dstATop),   
                image: new AssetImage("assets/themes/" + StoreProvider.of<AppState>(context).state.options.themeName + ".jpg"),
              ...
              child: NotificationListener<ScrollNotification> (
                child: CustomScrollView(
                  ...
                  SliverAppBar(
                    ...
                    snap: false,
                    floating: true,
                    expandedHeight: vm.showReaderOptions ? AppConstants.EXPANDED_APP_BAR : AppConstants.COLLAPSED_APP_BAR,
                    flexibleSpace: FlexibleSpaceBar(
                      title: Text((vm.showReaderOptions) ? AppConstants.EMPTY_STRING : vm.nitnemPathTitle),
                      ...
                      child: Padding(
                        ...
                        child: MediaQuery(
                          ...
                          child: Text(
                            ...
                            style: new TextStyle(
                              height: 2.0,
                              fontFamily: getLanguageMenuItemValueByName(vm.languageName).fontName,
                              fontSize: getLanguageMenuItemValueByName(vm.languageName).fontSize,
                              fontWeight: (vm.isBold) ? FontWeight.bold : FontWeight.normal,
                            ),
                          ),
                        ),
                      ),
        ...
        ? MediaQuery(
            ...
            child: new Container(
              child: Padding(
                padding: const EdgeInsets.all(AppConstants.STATUSBAR_PADDING),
                child: new Row(
                  children: <Widget>[
                    Expanded (
                      flex: 1,
                    ...
                    Expanded(
                      flex: 1,
                      child: Text (_calculateScrollPerc(vm.scrollOffset, vm.maxOffset) + "%",
          ...
          Visibility (
            ...
            child: FloatingActionButton.extended(
              onPressed: () {
                _controller.animateTo(0.0, duration: new Duration(milliseconds: 500), curve: Curves.ease);
              },
...
class _ViewModel {
  ...
  @override
  int get hashCode => runtimeType.hashCode ^ languageName.hashCode ^ showReaderOptions.hashCode
    ^ textScaleValue.hashCode ^ isBold.hashCode ^ showStatus.hashCode ^ pathData.hashCode ^ nitnemPathTitle.hashCode ^ scrollOffset.hashCode ^ statusTime.hashCode ^ batteryPerc.hashCode ^
